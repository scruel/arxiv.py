...
class Result(object):
    ...
    def __init__(
        ...
        self.links = links
        # Calculated members
        self.pdf_suffix = Result._get_pdf_suffix(links)
        # Debugging
        self._raw = _raw

    def _from_feed_entry(client, entry: feedparser.FeedParserDict) -> Result:
        """
    ...
    def _download(self, url: str, path: str, timeout=120):
        request = requests.get(
            url, timeout=timeout, stream=True)

        with open(path, 'wb') as fh:
            ftype = mime.from_buffer(request.content)
            fh.write(request.content)
        return path, ftype

    def download_pdf(self, path) -> str:
        """
        Downloads the PDF for this result to the specified directory.

        The filename is generated by calling `to_filename(self)`.
        """
        written_path, ftype = self._download('https://export.arxiv.org' + self.pdf_suffix, path)
        if ftype == 'application/pdf':
            return written_path
        # http://export.arxiv.org/pdf/2407.18219v1
        # pdf unavailable, try another way
        written_path, ftype = self._download('https://arxiv.org' + self.pdf_suffix, path)
        self.client.rate_limit_sleep()
        if ftype == 'application/pdf':
            return written_path
        raise LookupError(f'Got wrong type: {ftype}')

    def download_source(self, path: str) -> str:
        """
        ...
        source_suffix = self.pdf_suffix.replace("/pdf/", "/src/")
        written_path, ftype = self._download('https://export.arxiv.org' + source_suffix, path)
        if ftype == 'application/x-tar':
            return written_path
        # http://arxiv.org/src/2407.18219v1
        # got pdf rather than source, try another way
        written_path, ftype = self._download('https://arxiv.org' + self.pdf_suffix, path)
        self.client.rate_limit_sleep()
        if ftype == 'application/x-tar':
            return written_path
    ...
    def _get_pdf_suffix(links: List[Link]) -> str:
        """
        ...
        return (pdf_urls[0].removeprefix('http://').removeprefix('https://')
                .removeprefix('export.arxiv.org').removeprefix('arxiv.org'))
    ...
    def _results(self, search: Search, offset: int = 0) -> Generator[Result, None, None]:
        ...
        while feed.entries:
            for entry in feed.entries:
                try:
                    yield Result._from_feed_entry(self, entry)
